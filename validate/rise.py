...
def generate_masks(N, s, p1):
    cell_size = np.ceil(np.array((384, 384)) / s)
    up_size = (s + 1) * cell_size

    grid = np.random.rand(N, s, s) < p1
    grid = grid.astype('float32')

    masks = np.empty((N, 384, 384))
    ...
    for i in tqdm(range(N), desc='Generating masks'):
        ...
        masks[i, :, :] = resize(grid[i], up_size, order=1, mode='reflect',
                                anti_aliasing=False)[x:x + 384, y:y + 384]
    masks = masks.reshape(-1, 384, 384, 1)
    return masks
...
def explain(model, inp, masks):
    ...
    for i in tqdm(range(0, N, batch_size), desc='Explaining'):
        model_output = model(torch.tensor(masked[i:min(i+batch_size, N)].transpose(0, 3, 1, 2)).float())
        for j in range(len(model_output[0])):
            preds.append(model_output[0][j].detach().cpu().numpy())
    # preds = np.concatenate(preds)
    preds = np.array(preds)
    sal = preds.T.dot(masks.reshape(N, -1)).reshape(-1, 384, 384)
    sal = sal / N / p1
    return sal
