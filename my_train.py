...
def eval(
    ...
    with torch.no_grad():
        total = 0
        correct = 0
        for i, (images, masks, captions, labels, imp_path) in enumerate(
            tqdm(valid_loader)
        ):
            ...
            if split == "test":
                ...
                for i, attn_map in enumerate(attn_maps):
                    ...
                    save_attn_map(
                        attn_map,
                        f"{output_path}/test_attn_maps/{os.path.basename(imp_path[i])}",
                    )
...
def main(args):
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=cfg.SOLVER.IMS_PER_BATCH, shuffle=True, num_workers=4
    )
    valid_loader = DataLoader(
        valid_dataset, batch_size=cfg.SOLVER.IMS_PER_BATCH, shuffle=True, num_workers=4
    )
    test_loader = DataLoader(
        test_dataset, batch_size=cfg.SOLVER.IMS_PER_BATCH, shuffle=False, num_workers=4
    )
    best_epoch = 0
    for epoch in range(num_epochs // 2):
        ...
        arrucacy, loss, iou = eval(
            model, valid_loader, criterion, cfg.OUTPUT_DIR, with_mask=True, split="val"
        )
