...
# [GCC 9.4.0]
# Embedded file name: /home/initial/workspace/smilab23/graduation_research/SAN/san/model/san.py
# Compiled at: 2023-10-27 16:39:28
# Size of source mod 2**32: 14989 bytes
...
class SAN(nn.Module):
    ...
    @configurable
    def __init__(self, *, clip_visual_extractor, clip_rec_head, side_adapter_network, ov_classifier, criterion, size_divisibility, asymetric_input=True, clip_resolution=0.5, pixel_mean=[
 0.48145466, 0.4578275, 0.40821073], pixel_std=[
 0.26862954, 0.26130258, 0.27577711], sem_seg_postprocess_before_inference=False):
        super().__init__()
    ...
    def forward(self, batched_inputs):
        ...
        if 'vocabulary' in batched_inputs[0]:
            ov_classifier_weight = self.ov_classifier.logit_scale.exp() * self.ov_classifier.get_classifier_by_vocabulary(batched_inputs[0]['vocabulary'])
        else:
            ...
            ov_classifier_weight = self.ov_classifier.logit_scale.exp() * self.ov_classifier.get_classifier_by_dataset_name(dataset_names[0])
        if self.training:
        ...
        if self.asymetric_input:
            clip_input = F.interpolate(clip_input,
              scale_factor=(self.clip_resolution), mode='bilinear')
